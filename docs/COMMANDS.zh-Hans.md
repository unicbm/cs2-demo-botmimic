...
replay identity、武器/loadout 对齐、投掷物对齐和准星对齐默认开启。identity 对齐会通过
CounterStrikeSharp 的 player name 字段直接应用 demo 名字；SteamID64 和头像覆写仍然需要
BotHider 存在且管理目标 replay bot slot。
...
| `dtr_replay_identity` | `full` | 直接应用 demo 名字；SteamID64/头像对齐在可用时使用 BotHider 管理的 replay bot slot。 |
...
控制 replay identity 对齐。

开启后，加载 manifest 时会通过 `CCSPlayerController.PlayerName` 应用 demo 玩家
的 `player_name`。`full` 模式还会在 BotHider 可用且 slot 由 BotHider 管理时写入
SteamID64 和匹配的 PNG `avatar_overrides`，并启用 `sv_reliableavatardata`。默认模式是
`full`。

使用 `dtr_set identity sid` 时，只应用 demo SteamID64 和匹配的头像覆写，不应用 demo 名字。

`dtr_set identity name`、`visible` 和 `full` 都不会调用 `bh_setname`；它们使用的就是
简单 CounterStrikeSharp 改名插件那种直接赋值方式。

DemoTracer 特意使用这条 CounterStrikeSharp 改名路径，而不是 BotHider 更底层的
名字重写。另一种方案是要求本地 botprofile 覆盖所有 demo 选手名，让原生
`bot_kick <demo-name>` 可以直接命中；但 demo 来源复杂，每个本地服务器都维护这样一套
botprofile 很脆弱。因此目前保留普通显示名重写，并通过 `dtr_bots` 暴露精确的
`kickid` 提示。

这个主要用于 POV 和 spectator 观察清晰度。identity 对齐只会对 DemoTracer 选中的安全
replay bot slot 执行，不会应用到真人玩家。
...
是否可作为 replay candidate、slot、userid、短队名、显示名，以及用于精确踢人的
`kickid` 提示。

由于 DemoTracer 会重写 bot 的显示名，原生 `bot_kick "NiKo"` 可能不会按你看到的
demo 名字命中。此时应使用 `dtr_bots` 输出的唯一 server userid：

```text
[Client] slot=12 userid=12 team=CT bot=True managed=True replay=0 candidate=True name="m0NESY" kick_hint='kickid 12'
```

这个例子里，在控制台输入 `kickid 12` 就会定向踢出这个 m0NESY bot。需要注意的是，
根据你的本地服务器 bot quota/fill 设置，CS2 仍可能再补一个 bot 进来；但对“指定踢掉某个
replay bot”这个需求，`kickid` 提示是目前推荐的稳定路径。
