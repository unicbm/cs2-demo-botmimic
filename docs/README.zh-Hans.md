...
- 路线回放是高保真 replay，但不是完整离线 usercmd 还原。
- 投掷物、handoff、双架、BotHider 身份、头像和饰品对齐都有已知边界情况。
- 饰品/econ 导出和 runtime 对齐都是显式 opt-in，并带有服务器运营风险；见
  [饰品对齐与 GSLT 风险](#饰品对齐与-gslt-风险)。
- 这个工具面向本地服务器、研究、内容制作和插件开发，不用于匹配或作弊。

详细已知限制和边界情况见 [`LIMITATIONS.zh-Hans.md`](LIMITATIONS.zh-Hans.md)。
